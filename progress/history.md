...
## 2026-08-31 — 61_agente_scan (#61) implementada y aprobada — in_progress por T17 (campo)

- **Agente:** spec_author (cloud, junto con #60) → puerta humana 2026-08-27 (ventana propia Wails, app única con dependencias invisibles, macOS+Windows, Open-AudIT Community, sin firma en v1, repo aparte, GHCR, UAC único, sin auto-update) → implementer (cloud) → reviewer (cloud).
- **Resultado:** agente de escritorio completo en `sys-scan-agent/` (Wails v2 + Go + Svelte 5/Tailwind es-AR): 11 paquetes internos (`creds` keyring fail-closed, `logx` con redacción de secretos, `queue` SQLite con backoff, `sync` cliente #60, `nmaphost` ARP en host, `dockerx` contenedor efímero solo-localhost, `openaudit` cliente REST 6.x, `normalize` merge MAC ARP>OA>IP + clasificación de tipo, `scan` orquestador con reanudación y purga, `update`, `app`). Imagen `sys-openaudit` (Ubuntu 24.04 + Open-AudIT 6.0.4 pineado por SHA-256). Pieza auditapp: `scripts/export-escaneo-schema.ts` + `static/agente/` (schema JSON + version.json). CI: 4 workflows.
- **Desviación aceptada:** el agente vive temporalmente como subdirectorio autocontenido de auditapp (go.mod propio, cero imports al padre, CI propio) porque el repo destino no era accesible desde la VM; extracción documentada con `git filter-repo` al repo `martinmalgor04/sys-scan-agent` (creado por el usuario 2026-08-31, público; pendiente el push inicial — la instalación de Cursor solo tiene escritura en auditapp).
- **Verificación:** 72 tests Go verdes (10 paquetes, `-race` en scan/queue), cross-compile win/mac OK, `wails build` OK (17.5 MB), frontend svelte-check 0 errores, 15/15 tests auditapp nuevos, JSON Schema re-export reproducible, suite 1614 vs baseline 1599 con las 14 fallas preexistentes byte-idénticas (cero nuevas). Todo reproducido por el reviewer en VM limpia.
- **Veredicto:** APPROVED con condición de cierre — **T17 (prueba de campo en LAN real) bloquea el `done`**: requiere notebook en LAN física con ≥10 dispositivos. Checklist listo en `tasks.md` T17. La feature queda `in_progress` con T17 como único ítem abierto.
- **Próximo paso:** (1) prueba de campo T17 por el humano; (2) push inicial del repo `sys-scan-agent` (requiere PAT o acceso en la instalación de Cursor); (3) CI publica imagen `sys-openaudit` en GHCR y se pinea el digest en `dockerx`; (4) #63 (scoring) y #64 (diff) siguen pending.
