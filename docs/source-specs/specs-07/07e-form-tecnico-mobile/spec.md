...
El form se arma leyendo `template → section → template_item` de la auditoría ([07a](../07a-modelo-datos/spec.md)). Cada `field_type` mapea a un componente:
...
| `file_ref` | Botón "Tomar foto / subir" → cámara nativa → R2 ([07g](../07g-storage-r2/spec.md)) |
...
- v1: score **manual** (criterio del técnico). Autocálculo desde los ítems queda para v2 ([07f §8](../07f-cierre-auditoria/spec.md)).
...
- Cuando el técnico marca el relevamiento completo → `audit.status = en_cierre` y pasa a la pantalla de cierre ([07f](../07f-cierre-auditoria/spec.md)).
...
- Datos y `field_type`: [SPEC-07a](../07a-modelo-datos/spec.md). Permisos (técnico ve solo lo asignado): [SPEC-07b](../07b-auth-roles/spec.md).
- Fotos/exports: [SPEC-07g](../07g-storage-r2/spec.md). Cierre: [SPEC-07f](../07f-cierre-auditoria/spec.md).
- Motor de render compartido con el briefing ([07d](../07d-briefing-externo/spec.md)).
- Marca: skill `sys-brand`. PWA/SW: [SPEC-07h](../07h-stack-deploy/spec.md).
...
- [ ] Tamaño/compresión de fotos antes de subir (ahorro de datos) — ver [07g](../07g-storage-r2/spec.md).
