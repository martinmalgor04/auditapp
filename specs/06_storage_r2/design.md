# Design — #6 06_storage_r2

## Alcance

Módulo server de almacenamiento en Cloudflare R2: generación de presigned PUT/GET con `aws4fetch`, convención de keys, persistencia en `attachment` y vínculo con `audit_response` para ítems `file_ref`. Endpoints API protegidos por sesión (depende de `03_auth_roles` #3).

| Incluido | Excluido (otras features) |
|---|---|
| `aws4fetch` + módulo `storage/` | Compresión cliente 1600px/0.8 (`07_form_tecnico` #7) |
...
| Feature | Qué aporta |
|---|---|
| `01_stack_scaffolding` (#1) | Carpeta `src/lib/server/storage/`, `.env.example` vars R2 |
| `02_modelo_datos` (#2) | Tabla `attachment`, `audit_response`, constraints |
| `03_auth_roles` (#3) | `event.locals.user`, guards técnico/admin |
...
- Cron/worker en feature futura o `10_deploy_dokploy` #10.
