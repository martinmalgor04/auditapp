...
- [x] T1 — Completar el cuerpo de la plantilla **`envio_informe_cliente`** en
...
- [x] T2 — Extender `tests/email-templates.test.ts`: render de `envio_informe_cliente` produce
...
- [x] T3 — Crear `src/lib/server/informe/enviar.ts`: `enviarInformeSchema` (Zod `to` email),
...
- [x] T4 — `tests/informe-enviar.test.ts`: schema acepta/rechaza destinatario (R3); `data` armado
  no incluye `clientDraft`/`upsell`/`internal` (R4); `listInformeEnvios` devuelve destinatario+fecha
  ordenados (R7). Cubre: R3, R4, R7.

- [x] T5 — Crear endpoint `POST /api/audits/[id]/report/[version]/enviar`: carga audit/report/empresa,
...
- [x] T6 — `tests/api/informe-enviar.test.ts`: 401 sin sesión, 403 técnico no asignado, 200 admin y
...
- [x] T7 — `tests/informe-enviado.test.ts`: marca "enviado" derivada de `email_log` (destinatario +
  fecha); reenvío agrega fila. Cubre: R7.

- [x] T8 — Extender `load` de `.../informe/[version]/+page.server.ts`: con `status==='aprobado'`,
  exponer `empresa.email` (prefill) y `listInformeEnvios(report.id, empresa.email)` (marca). Cubre:
  R1, R7.

- [x] T9 — Crear `src/lib/components/informe/enviar-informe-dialog.svelte` (modal confirmación,
...
- [x] T10 — `e2e/envio-informe-email.spec.ts`: informe aprobado (fixture #14 `INFORME_FAKE=1`) →
...
- [x] T11 — Verificación final: `pnpm run check`, `pnpm test` y
