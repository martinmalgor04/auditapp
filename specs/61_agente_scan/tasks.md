...
- [ ] T1 — (auditapp) Crear `scripts/export-escaneo-schema.ts` que exporte
  `dispositivo-input.schema.json` desde los Zod de #59
  (`zod-to-json-schema`). Cubre: R25.

- [ ] T2 — (auditapp) Crear `static/agente/version.json` con la versión
  inicial del agente y placeholders de URLs de descarga. Cubre: R29.

- [ ] T3 — Scaffolding del agente: repo `sys-scan-agent`, Wails v2 + Go +
...
- [ ] T4 — `internal/creds`: `CredStore` con `99designs/keyring`
...
- [ ] T5 — `internal/queue`: cola SQLite (`modernc.org/sqlite`) con schema
...
- [ ] T6 — `internal/sync`: cliente HTTP de #60 (GET estado, consentimiento,
...
- [ ] T7 — `internal/nmaphost`: barrido ARP `nmap -PR -sn` en host con
...
- [ ] T8 — Imagen `sys-openaudit`: Dockerfile Ubuntu 24.04 + instalador
  oficial Open-AudIT pineado (versión + SHA-256), healthcheck, CI con build
  multi-arch (amd64/arm64) y push a GHCR. Cubre: R21, R22.

- [ ] T9 — `internal/dockerx`: pull con verificación de digest pineado y
...
- [ ] T10 — `internal/openaudit`: cliente REST (healthcheck, credentials
...
- [ ] T11 — `internal/normalize`: mapeo OA/Nmap → `dispositivoInput` según
...
- [ ] T12 — `internal/scan`: orquestador de fases (validar token →
...
- [ ] T13 — UI del agente (Svelte): pantallas de token/confirmación,
...
- [ ] T14 — Cierre y purga: al cerrar escaneo (o limpieza de arranque)
...
- [ ] T15 — Empaquetado y firma en CI: `.exe` NSIS firmado (certificado
...
- [ ] T16 — Test de integración en CI: contenedor `sys-openaudit` real + red
