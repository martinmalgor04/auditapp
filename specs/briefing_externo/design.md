...
| Validación `public_token` + estados | Generación/regeneración de token (#4 backoffice) |
...
|---|---|
| `modelo_datos` | Tablas `audit`, `audit_response`, `template_item`, `client`; UNIQUE `(audit_id, item_id)` |
| `auth_roles` | Resolución de token; `hooks.server.ts` NO exige sesión en `/briefing/*` |
| `backoffice` | Auditoría en `briefing_enviado` con `public_token` y `token_expires_at` poblados |
...
Documentar en `progress/impl_briefing_externo.md`:
