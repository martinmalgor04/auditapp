...
Sin scripts de init SQL en esta feature (el schema llega en `02_modelo_datos` #2). El Dockerfile fija usuario/DB por defecto; `docker-compose.yml` puede repetir las mismas vars para claridad.
...
**Motivo:** PWA completa (manifest SyS, SW shell) pertenece a `07_form_tecnico` (#7) y `10_deploy_dokploy` (#10). Incluirla ahora mezalaría alcance.
