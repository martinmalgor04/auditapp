...
| Feature | Qué aporta |
|---|---|
| `02_modelo_datos` (#2) | Tablas, rúbrica en `options`, estados, `audit_closure` sin `indice_global` |
| `03_auth_roles` (#3) | Guards, invalidación token pattern |
| `04_backoffice` (#4) | Ruta base `auditorias/[id]`, transición relevamiento→cierre |
| `07_form_tecnico` (#7) | Consume `computeLiveScores`; observaciones por sección |
| `09_contrato_datos` (#9) | Reutilizará el mismo motor + builder JSON (futuro) |
...
`buildClosurePreview` compone la vista humana. El JSON canónico con `schema_version`, `score_basis`, `market_data` y endpoint protegido lo implementa `09_contrato_datos` (#9) importando `scoreAudit` y ampliando el payload. No duplicar schema acá.
