...
- [x] T4 — Tests de contrato en `tests/canonical-schema.test.ts` y
...
- [x] T5 — Test en `tests/canonical-builder.test.ts`: ítem table con filas y
...
- [x] T6 — Crear `src/lib/informe/inventory-columns.ts` con
...
- [x] T7 — Test unitario `tests/informe/inventory-columns.test.ts`: mapea
  columnas tipo/modelo/antigüedad/EOL; rechaza tablas no-inventario y dominio ERP. Cubre: R8, R9.

## Modelo de render

- [x] T8 — En `src/lib/informe/render-shared.ts`, extender `InformeRenderModel`
  con `inventarioIt: Array<{ tipo, modeloCategoria, antiguedad, estadoEol, semaforo, fotos }>`. Cubre: R6.

- [x] T9 — En `src/lib/server/informe/model.ts`, derivar `inventarioIt` desde el
...
- [x] T10 — Añadir a `options` de `buildInformeRenderModel` el resolvedor
...
- [x] T11 — En `src/lib/informe/web-render.ts`, agregar `renderInventario(model)`
...
- [x] T12 — Insertar `renderInventario(model)` en `renderInformeWebHtml` después
  de `renderHallazgos` (it/mixta). Cubre: R8.

- [x] T13 — Portar al `STYLE` de `web-render.ts` las clases `.equip*` del gold
...
- [x] T14 — En `tests/informe-web-render.test.ts`: nuevo snapshot inventario IT
  **con** fotos (URLs deterministas vía resolvedor fake) y **sin** fotos
  (placeholder). Cubre: R8, R10, R11.

- [x] T15 — En `tests/informe-web-render.test.ts`: verificar que el snapshot ERP
  puro existente no cambia y que ERP puro no incluye la sección inventario. Cubre: R9, R14.

- [x] T16 — En `tests/informe-web-render.test.ts`: aserción de no-fuga — el HTML
...
- [x] T17 — Ejecutar `pnpm run check`, `pnpm test` y `./init.sh` en verde;
