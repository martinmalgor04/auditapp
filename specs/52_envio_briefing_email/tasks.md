...
- [x] T1 — Completar el `render` de la plantilla `envio_briefing_cliente` en
...
- [x] T2 — `tests/envio-briefing-email-template.test.ts`: render con datos válidos
...
- [x] T3 — Crear `src/lib/server/backoffice/briefing-email.ts`:
...
- [x] T4 — En el mismo archivo, `getBriefingEmailMark(auditId)`: deriva la marca del
...
- [x] T5 — `tests/api/envio-briefing-email-send.test.ts` (dry-run / transporte mock #49):
...
- [x] T6 — Extender `src/routes/(app)/auditorias/[id]/+page.server.ts`:
...
- [x] T7 — `tests/envio-briefing-email-enable.test.ts`: matriz de `canSendBriefingEmail`
  (token vigente + email → true; sin token / `borrador` / `cerrada` / sin email → false).
  Cubre: R2.

- [x] T8 — Extender `src/routes/(app)/auditorias/[id]/+page.svelte`: botón "Enviar
...
- [x] T9 — `tests/envio-briefing-email-schema.test.ts`: verifica que la marca (R6) se
...
- [x] T10 — `e2e/envio-briefing-email.spec.ts`: fixture de auditoría en `briefing_enviado`
