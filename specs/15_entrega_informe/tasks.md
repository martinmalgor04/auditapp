...
- [ ] T1 — Crear migración `migrations/006_entrega_informe.sql`: tabla `audit_report_share`
...
- [ ] T2 — Implementar `src/lib/server/db/informe-shares.ts`:
...
- [ ] T3 — Implementar `src/lib/server/informe/share.ts`: `generateShareToken` (patrón
...
- [ ] T4 — `src/routes/api/audits/[id]/report/[version]/share/+server.ts`: POST (201, 400
...
- [ ] T5 — `src/routes/informe/[token]/`: `+layout.svelte` público sin chrome con meta
...
- [ ] T6 — Implementar `src/lib/components/informe/report-web-render.svelte` según
  ...
  Cubre: **R10, R11, R12**.
- [ ] T7 — Implementar `src/lib/client/informe/web-effects.ts` (scroll progress, reveal con
...
- [ ] T8 — `src/routes/informe/[token]/imprimir/`: `+page.server.ts` con la misma validación
...
- [ ] T9 — Implementar `src/lib/components/informe/share-panel.svelte` y montarlo en
...
- [ ] T10 — `tests/informe-share.test.ts`: token 43 chars base64url y unicidad,
...
- [ ] T11 — `tests/informe-web-render.test.ts`: snapshot con fixture canónico estable —
...
- [ ] T12 — `tests/api/informe-share-admin.test.ts`: sin sesión 401, `tecnico` 403 en
  ...
  R6, R7, R8, R9**.
- [ ] T13 — `tests/api/informe-share-public.test.ts`: GET con token vigente 200 sin sesión
...
- [ ] T14 — `e2e/entrega-informe.spec.ts` (flujo #14 con Claude fake hasta `aprobado`):
...
- [ ] T15 — Ejecutar `pnpm test` (suites share/web-render verdes, sin servicios externos) y
  `pnpm exec playwright test e2e/entrega-informe.spec.ts`. Cubre: **R15, R16**.
- [ ] T16 — Ejecutar `./init.sh` exit 0, `pnpm run check` 0 errores y completar la
