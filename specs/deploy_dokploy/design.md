...
| Feature | Aporta |
|---|---|
| `stack_scaffolding` (#1) | `adapter-node`, `package.json` scripts, `.env.example` base, estructura `migrations/` |
| `modelo_datos` (#2) | `runMigrations()`, `runSeed()`, `pnpm run db:migrate`, `pnpm run db:seed`, `schema_migration` |
| `auth_roles` (#3) | Cookies de sesión; flag `Secure` condicionado a HTTPS |
| `form_tecnico` (#7) | `@vite-pwa/sveltekit`, manifest SyS, SW shell — esta feature verifica que se sirvan en prod |
