...
> Milestones 8b–8d de SPEC-07h. Complementa `stack_scaffolding` (#1).
> Depende de: `stack_scaffolding` (#1), `modelo_datos` (#2) para runner `migrate.ts` y `runSeed`; PWA plugin de `form_tecnico` (#7) para assets en prod.
> Fuentes: `docs/source-specs/specs-07/07h-stack-deploy/spec.md`, `docs/source-specs/prds/auditapp-07h-stack-deploy.prd.md`.
...
CUANDO un operador ejecuta el comando de seed documentado contra la DB de producción con migraciones ya aplicadas, el sistema DEBE completar el seed idempotente definido en `modelo_datos` (#2) sin error.
