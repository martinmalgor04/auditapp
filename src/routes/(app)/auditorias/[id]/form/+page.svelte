<script lang="ts">
  ...
  // T21/T23 — datos para FormHeader, SectionChips, FormNextButton
  const totalItems = $derived(data.sections.reduce((acc, s) => acc + s.items.length, 0));
  const totalDone = $derived(
    data.sections.reduce((acc, s) => {
      const prog = progressBySec.get(s.id);
      return acc + (prog ? prog.answered : 0);
    }, 0)
  );
  const globalProgress = $derived(totalItems > 0 ? Math.round((totalDone / totalItems) * 100) : 0);
  const globalPending = $derived(totalItems - totalDone);

  const sectionChipsData = $derived(
    data.sections.map((s) => {
      const prog = progressBySec.get(s.id);
      return {
        code: s.code,
        done: prog ? prog.answered : 0,
        total: s.items.length
      };
    })
  );

  const activeSectionPending = $derived(
    activeSection
      ? activeSection.items.filter((it) => {
          const st = itemStatuses.get(it.id) ?? 'pendiente';
          return st === 'pendiente';
        }).length
      : 0
  );
...
<!-- T23 — SectionChips debajo del header -->
<div class="lg:hidden sticky top-[72px] z-30 -mx-4 bg-sys-offwhite py-2 border-b border-sys-medio/10">
  <SectionChips
    sections={sectionChipsData}
...
<div class="pt-28 lg:pt-0 lg:grid lg:grid-cols-[280px_1fr] lg:gap-6">
...
<!-- T23 — FormNextButton flotante mobile -->
<FormNextButton pendingCount={activeSectionPending} onClick={goToNextPending} />
