<script lang="ts">
  ...
  import StatCard from '$lib/components/mercado/stat-card.svelte';
...
<div class="mx-auto max-w-6xl space-y-6 p-4 sm:p-6">
  ...
  {:else}
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <StatCard
        title="Índice IT promedio"
        value={data.dashboard.indices_global.avg_it}
        subtitle="n = {data.dashboard.indices_global.n_it}"
        testId="mercado-stat-it"
      />
      <StatCard
        title="Índice ERP promedio"
        value={data.dashboard.indices_global.avg_erp}
        subtitle="n = {data.dashboard.indices_global.n_erp}"
        testId="mercado-stat-erp"
      />
      <StatCard
        title="Auditorías cerradas"
        value={data.dashboard.universe.n}
        testId="mercado-stat-universe"
      />
      <StatCard
        title="Con hallazgos upsell"
    ...
    <section class="rounded-sys border border-sys-borde bg-white p-4 shadow-sm" data-testid="mercado-section-erp">
      ...
      <BarChart items={data.dashboard.erp_distribution} testId="mercado-erp-chart" />
    </section>
    ...
    >
      ...
      <div class="mt-4 grid gap-4 sm:grid-cols-3">
        <StatCard title="Total hallazgos" value={data.dashboard.upsell_internal.total} />
        <StatCard
          title="Promedio por auditoría"
          value={data.dashboard.upsell_internal.avg_per_audit?.toFixed(2) ?? null}
        />
        <StatCard
          title="Auditorías con hallazgos"
          value={data.dashboard.upsell_internal.audits_with_findings}
        />
      </div>
    </section>
