 ...
 */
export async function processReunionJobDirect(sessionId: string): Promise<void> {
  ...
  if (!session.attachment_id) {
    throw new Error(`Sesión ${sessionId} no tiene audio confirmado`);
  }

  const attachment = await getAttachmentById(session.attachment_id);
  if (!attachment) {
    throw new Error(`Attachment ${session.attachment_id} no encontrado`);
  }

  // 1. Insertar transcript en pending
