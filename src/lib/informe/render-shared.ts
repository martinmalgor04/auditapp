/**
 * Utilidades compartidas del render A4 (#14, #19): STYLE, helpers y páginas reutilizables.
 */
...
export function tituloPortada(tipo: InformeRenderModel['tipoAuditoria']): string {
  if (tipo === 'it') return 'Auditoría<br>IT';
  if (tipo === 'mixta') return 'Auditoría<br>IT + ERP';
  return 'Auditoría<br>ERP';
}

export function tipoLabel(tipo: InformeRenderModel['tipoAuditoria']): string {
...
export function footer(pagenum: string): string {
  return `<div class="footer"><img src="${LOGO_COLOR_URL}" alt="SyS"><span class="pagenum">${pagenum}</span></div>`;
}

export function renderStatCircuitos(model: InformeRenderModel, opts: RenderOptions): string {
...
export function renderGaugeErp(model: InformeRenderModel): string {
  ...
  return renderGaugeSvg(index, label);
}

export function renderGaugeIt(model: InformeRenderModel): string {
  return renderGaugeSvg(model.draft.indices.it, 'Índice IT general');
}

export function renderGaugeErpOnly(model: InformeRenderModel): string {
  return renderGaugeSvg(model.draft.indices.erp, 'Índice ERP general');
}

export function countItAreasRelevadas(model: InformeRenderModel): number {
  return model.secciones.filter(
    (s) => s.domain === 'it' && s.code !== 'CAB' && s.score !== null
  ).length;
}

export function sectionByCodeMap(model: InformeRenderModel): Map<string, InformeRenderModel['secciones'][number]> {
  return new Map(model.secciones.map((s) => [s.code, s]));
}

export function domainForSeccionCode(
...
export function renderHallazgosFilas(
  ...
  const circuitos = model.draft.hallazgos.circuitos.filter((c) => {
    if (filterDomain === undefined) return true;
    const domain = domainForSeccionCode(model, c.seccion_code);
...
export function renderLecturaTransversal(d: RenderClientDraft, opts: RenderOptions): string {
  return d.hallazgos.lectura_transversal
    .map(
      (l, i) =>
...
export function renderRiesgosPage(
  model: InformeRenderModel,
  opts: RenderOptions,
  pagenum: string,
  eyebrow: string
...
export function renderCierrePage(
  model: InformeRenderModel,
  opts: RenderOptions,
  eyebrow: string,
  pagenum?: string
...
<section class="page dark backcover">
  ...
  <div style="text-align:left; width:100%; margin-bottom:16mm;">
    <div class="eyebrow" style="color:var(--sys-celeste);">${eyebrow}</div>
...
export function renderCircuitoCards(
  model: InformeRenderModel,
  opts: RenderOptions,
  filterDomain?: 'it' | 'erp'
): string {
  const sectionByCode = sectionByCodeMap(model);
  const circuitos = model.draft.dia_a_dia.circuitos.filter((c) => {
    if (filterDomain === undefined) return true;
    return domainForSeccionCode(model, c.seccion_code) === filterDomain;
  });
...
export function renderLoomBlock(model: InformeRenderModel): string {
  const loomId = model.loomUrl ? model.loomUrl.split('/').pop() : null;
  if (!model.loomUrl || !loomId) return '';
  return `<div class="informe-loom"><iframe src="https://www.loom.com/embed/${e(loomId)}" allowfullscreen title="Video Loom"></iframe></div>`;
}

export function wrapInforme(template: string, pages: string, loom: string): string {
  return `${STYLE}
<div class="informe-a4" data-template="${template}">
${pages}
${loom}
</div>`;
}
